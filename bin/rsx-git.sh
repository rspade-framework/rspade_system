...
reset_system_tree() {
    ...
    # deterministically, every re-run (Ascent, 2026-08-18). The notice claimed the
...
# applies silently and no conflict resolver is ever invoked for (Ascent, 2026-08-11 and
...
# (Ascent, 2026-08-18). It is a LOG; the tree it logs decides its content.
restate_system_tree() {
...
assert_release_not_regressed() {
    ...
    err "        through shared history (Ascent, 2026-08-11) - it is a loss whether or not"
...
run_maintenance_op() {
    ...
    # box reported (Ascent, 2026-08-12: no lock file, no concurrent git, clean status).
