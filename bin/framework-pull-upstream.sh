...
#   the older marker, untracking 7,503 paths (a downstream deployment). The marker travels inside the
...
three_way_pass() {
    ...
    while IFS= read -r rel; do
        ...
        if [ "$new_exists" = true ] && [ "$old_exists" = true ] && [ "$disk_exists" = true ]; then
            ...
            # absence is how 13 framework migrations went missing on a downstream deployment (2026-08-11)
    ...
    # migrations hid behind 7,326 correctly-deleted fixtures on a downstream deployment while the run
...
# the next verify false-flags all of that churn forever (the downstream field failure).
...
# concurrent auto-rebuild for the build lock (the downstream field failure: the prune
...
print_summary() {
    ...
    # unmentioned; a downstream deployment, 2026-08-18). The final block is what an operator retains, so
...
main() {
    ...
    # framework churn forever (B-52, the downstream field failure); the reconciling prune
