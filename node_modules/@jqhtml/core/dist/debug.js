...
// Log lifecycle event
export function logLifecycle(component, phase, status) {
    const jqhtml = getJqhtml();
    if (!jqhtml?.debug)
        return;
...
export function applyDebugDelay(phase) {
    const jqhtml = getJqhtml();
    if (!jqhtml?.debug)
        return;
    let delayMs = 0;
...
export function logInstruction(type, data) {
    const jqhtml = getJqhtml();
...
// Log data changes
export function logDataChange(component, property, oldValue, newValue) {
    const jqhtml = getJqhtml();
...
export function logDispatch(url, route, params, verbose = false) {
    const jqhtml = getJqhtml();
    if (!jqhtml?.debug)
        return;
...
export function isSequentialProcessing() {
    const jqhtml = getJqhtml();
    return jqhtml?.debug?.sequentialProcessing || false;
}
// Error handling with break on error
export function handleComponentError(component, phase, error) {
    const jqhtml = getJqhtml();
