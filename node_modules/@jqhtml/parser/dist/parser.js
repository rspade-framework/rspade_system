...
export class Parser {
    ...
    parse_component_definition() {
        ...
        if (!is_component_name(name_token.value)) {
            throw syntaxError(`Component name '${name_token.value}' ${COMPONENT_NAME_RULE}. Convention is First_Letter_With_Underscores.`, name_token.line, name_token.column, this.source, this.filename);
        }
     ...
     */
    parse_slot_params(name_token, attributes, conditionalAttributes) {
        const fail = (message) => syntaxError(message, name_token.line, name_token.column, this.source, this.filename);
        if (conditionalAttributes.length > 0) {
            throw fail(`Slot "${name_token.value}" cannot have conditional attributes`);
        }
        for (const key of Object.keys(attributes)) {
            if (key !== '$params') {
                throw fail(`Slot "${name_token.value}" does not accept the attribute "${key}". ` +
        ...
        const params = raw.value.split(',').map((p) => p.trim()).filter((p) => p.length > 0);
        if (params.length === 0) {
    ...
    parse_tag() {
        ...
        if (is_dynamic && name_token.value.trim() === '') {
            throw syntaxError(`Dynamic component tag <{}> has an empty expression`, name_token.line, name_token.column, this.source, this.filename);
        }
        ...
        while (!(is_dynamic ? this.check_closing_dynamic_tag(tag_name) : this.check_closing_tag(tag_name)) &&
            ...
            if (this.is_at_end()) {
                const error = unclosedError(is_component ? 'component' : 'tag', is_dynamic ? `{${dynamic_expression}}` : tag_name, start_token.line, start_token.column, this.source, this.filename);
                throw error;
            }
            ...
            if (is_dynamic && this.check(TokenType.TAG_CLOSE)) {
                ...
                if (next && next.type === TokenType.DYNAMIC_TAG_NAME) {
                    throw syntaxError(`Mismatched dynamic component tags: <{${dynamic_expression.trim()}}> opened on line ${start_token.line} ` +
                        `is closed by </{${next.value.trim()}}> on line ${next.line}. ` +
                        `The closing expression must match the opening one textually (whitespace aside).`, next.line, next.column, this.source, this.filename);
                }
            }
        ...
        if (is_dynamic)
            close_name.value = Parser.normalize_expression(close_name.value);
    ...
    static normalize_expression(expression) {
        return expression.trim().replace(/\s+/g, ' ');
    }
    check_closing_dynamic_tag(normalized_expression) {
