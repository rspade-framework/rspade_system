...
export class CodeGenerator {
     ...
     */
    merge_output_pushes(line) {
        const PUSH = '_output.push(';
        if (line.indexOf(PUSH) === -1) {
            return line;
        }
        let result = '';
        let pending = []; // argument text of the run being accumulated
     ...
     */
    find_matching_paren(text, open_index) {
        let depth = 0;
        let quote = null;
    ...
    generate_raw_tag_content(node) {
        const parts = [];
        for (const child of node.children) {
            if (child.type === NodeType.TEXT) {
                parts.push(this.escape_string(child.content));
            }
