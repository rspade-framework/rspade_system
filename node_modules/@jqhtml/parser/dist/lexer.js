...
(function (TokenType) {
    ...
    TokenType["DYNAMIC_TAG_NAME"] = "DYNAMIC_TAG_NAME";
...
export class Lexer {
    ...
    scan_dynamic_tag(closing) {
        ...
        if (depth !== 0) {
            throw new JQHTMLParseError(`Unterminated dynamic component tag: <${closing ? '/' : ''}{ has no matching }`, start_line, start_column, this.input);
        }
     ...
     */
    scan_raw_text_element(tag_name) {
    ...
    scan_interpolated_attribute_value(quote) {
        ...
        while (this.position < this.input.length && this.current_char() !== quote) {
            ...
            if (this.current_char() === '<' && this.peek_ahead(1) === '%') {
                ...
                else if (this.peek_ahead(2) === ' ' || this.peek_ahead(2) === '\t' ||
                    this.peek_ahead(2) === '\n' || this.peek_ahead(2) === '\r') {
    ...
    attribute_code_block_error() {
        const attr_context = this.get_current_attribute_context(); // e.g. "style="
        const error = new JQHTMLParseError(`<% %> code blocks are not allowed inside attribute values.\n` +
            `  Attribute: ${attr_context}"<% ...`, this.line, this.column, this.input);
