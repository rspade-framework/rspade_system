...
function generateSourcemapForWrappedCode(wrappedCode, sourceContent, filename) {
    ...
    // Remaining source lines map sequentially, BOUNDED BY THE GENERATED OUTPUT: a Define whose body renders nothing compiles to a one-line render function, so the source can be far longer than the code (a long <%-- --%> header is the usual way). A segment for a generated line that does not exist is a phantom line, and Mozilla's SourceNode.fromStringWithSourceMap materializes each phantom as a bare `undefined` identifier when the bundle is concatenated.
