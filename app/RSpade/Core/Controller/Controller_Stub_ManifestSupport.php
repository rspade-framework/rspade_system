 ...
 * `My_Controller.save({...})` exists in the browser with no wiring.
 *
 * AN ORDINARY SUPPORT MODULE. Stub generation used to be dispatched separately, by a
 * method_exists() sweep over the integration registry after every module had run; it is now
 * the same ordered list, and the phase order (stub generators LAST) is the list's order.
 *
 * NOT incremental by changed set - it walks the file map once - but it WRITES NOTHING it
 * does not have to: the desired content is compared against what is on disk before every
 * write, so a rebuild that changes no controller rewrites no stub and no bundle recompiles
 * because of a churned mtime.
 */
class Controller_Stub_ManifestSupport extends ManifestSupport_Abstract
{
    public static function get_name(): string
    {
        return 'Controller JS Stubs';
    }
    ...
    public static function process(array &$manifest_data, array $changed_files, array $removed_files): void
    {
        ...
        foreach ($manifest_data['data']['files'] as $file_path => &$metadata) {
            ...
            if ($needs_regeneration) {
                ...
                // the mtime, and a moved mtime makes every bundle carrying this stub
                // recompile. The gate above is a cheap pre-filter; this is the answer.
