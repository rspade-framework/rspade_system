...
/**
 * Sms_Queue_Model_Abstract - Database-backed outgoing SMS queue (framework core).
 *
 ...
 * is the message content. Core infrastructure the Rsx_Sms facade depends on. An app
 * customizes it by declaring `class Sms_Queue_Model extends Sms_Queue_Model_Abstract`
 * in rsx/models/, carrying only the members it changes.
 *
 * @property integer $id
 ...
 * @mixin \Eloquent
 *
 * THE BASE OF A SPLIT MODEL. Every member of the framework's Sms_Queue_Model lives here;
 * `Sms_Queue_Model.php` beside it is a shell an application replaces by declaring
 * `class Sms_Queue_Model extends Sms_Queue_Model_Abstract` under rsx/models/ - so an
 ...
 * @mixin \Eloquent
 */
abstract class Sms_Queue_Model_Abstract extends Rsx_Site_Model_Abstract
{
    /**
