 ...
 *
 * THE BASE OF A SPLIT MODEL. Every member of the framework's User_Model lives here;
 * `Core/Models/User_Model.php` is a three-line concrete an application replaces by
 * declaring `class User_Model extends User_Model_Abstract` under rsx/models/. That is
 * what keeps an application's customization down to the members it actually changes,
 * and what stops a frozen clone from missing the members the framework adds next.
 *
 * See: php artisan rsx:man acls, php artisan rsx:man class_override
 */
...
abstract class User_Model_Abstract extends Rsx_Site_Actor_Model_Abstract
{
     ...
     *
     * THE FRAMEWORK CANNOT KNOW THE ANSWER. A profile link names a SCREEN, and screens are
     * the application's: which route shows a staff user, whether the viewer's own record has
     * a different destination, and which gate guards each one are all decisions the base has
     * no way to make. So the question is asked of the application through the resolve hook
     * `user.view_profile_url` with `{user}`; the first handler returning a string wins, and
     * with no handler (or every handler declining) the answer is null, and <Record_Author>
     * renders the name as plain text - the correct behaviour for an application that has not
     * declared a destination. The reference app answers it in
     * `rsx/handlers/User_Profile_Url_Handlers.php`, resolving each destination through
     * Auth_Gates::accessible_route() so the link is present exactly when the viewer may
     * follow it.
     *
     * A hook rather than a class override on purpose: a seam beats a clone whenever the
     * addition is policy (rsx:man class_override), and an application that prefers to
     * override the concrete may still do so - the method is #[Replaceable].
     *
    ...
    {
        $resolved = Rsx::trigger_resolve('user.view_profile_url', ['user' => $this]);
