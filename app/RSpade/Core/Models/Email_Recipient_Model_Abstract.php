...
/**
 * Email_Recipient_Model_Abstract - Per-address tracking and unsubscribe management (core).
 *
 ...
 * Rsx_Mail blocklist API depends on. An app customizes it by declaring
 * `class Email_Recipient_Model extends Email_Recipient_Model_Abstract` in rsx/models/,
 * carrying only the members it changes.
 *
 * @property integer $id
 ...
 * @mixin \Eloquent
 *
 * THE BASE OF A SPLIT MODEL. Every member of the framework's Email_Recipient_Model lives here;
 * `Email_Recipient_Model.php` beside it is a shell an application replaces by declaring
 * `class Email_Recipient_Model extends Email_Recipient_Model_Abstract` under rsx/models/ - so an
 ...
 * @mixin \Eloquent
 */
abstract class Email_Recipient_Model_Abstract extends Rsx_Site_Model_Abstract
{
    ...
    public static function find_or_create_by_email(int $site_id, string $email): self
    {
        ...
        return static::without_site_scope(function () use ($site_id, $email) {
