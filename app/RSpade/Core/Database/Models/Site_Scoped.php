...
/**
 * Site_Scoped - the tenant boundary, as an adoptable trait.
 *
 * THIS IS THE IMPLEMENTATION. Rsx_Site_Model_Abstract is a model that adopts it and
 * declares nothing else, so "extend Rsx_Site_Model_Abstract" and "use Site_Scoped"
 * install exactly the same controls from exactly the same source.
 *
 * WHY IT IS A TRAIT AND NOT ONLY A BASE CLASS. A core model is EXTENDED, never copied:
 * the framework ships X_Model_Abstract carrying every member plus an empty X_Model shell,
 * and an application override must extend that base DIRECTLY. That fixes the base's own
 * parent, so when a framework table is not site-scoped but the application's copy of it IS
 * - the application added site_id in its own migration as part of a tenant sweep - the
 * override cannot reach Rsx_Site_Model_Abstract, and PHP has no second inheritance slot to
 * reach it with. Before this trait the only way out was to re-implement the create stamp
 * and the read scope by hand in the override, which is the cloned-machinery drift the split
 * exists to end. Now it is one line:
 *
 *     class User_Profile_Model extends User_Profile_Model_Abstract
 *     {
 *         use Site_Scoped;          // the framework's, not a copy of it
 *     }
 *
 * The adopting model's table MUST have a site_id column; every control below names it.
 *
 * WHAT IT INSTALLS (see bootSite_Scoped): the site read-isolation global scope, the
 * creating hook that forces site_id from the current experience, the saving hook that
 * refuses a cross-site or site-changing write, and the defensive retrieved cross-check -
 * plus save()/update()/delete() guards and a tenant-pinned attachment reader.
 *
 * INSTALLATION RIDES ON THE TRAIT BOOT, NOT ON booted(). Eloquent calls
 * bootSite_Scoped() from Model::bootTraits() for every class that adopts the trait, once,
 * before booted(). That is deliberately stronger than where this used to live: the
 * protections were installed in Rsx_Site_Model_Abstract::booted(), so a subclass that
 * overrode booted() and forgot parent::booted() silently dropped all of them - a fail-open
 * cross-tenant read defect guarded only by PHP-PARENT-CHAIN-01 catching the missing chain.
 * A trait boot cannot be forgotten by an override, so the isolation no longer depends on
 * anyone remembering to chain.
 *
 * $apply_site_scope IS PER ADOPTING CLASS, and that matters. A trait's static properties
 * are copied into each using class, so Rsx_Site_Model_Abstract holds ONE flag shared by
 * every model beneath it - identical to how this behaved when the property lived on that
 * class - while a model that adopts the trait directly holds its own. So
 * without_site_scope() on a direct adopter suspends that model only, and never reaches the
 * rest of the family. Read __attachment_query below with that in mind: its defense is
 * against the SHARED flag, and it stays correct either way.
 *
 * $site_lock_tokens is per adopting class for the same reason, which would matter if the
 * automatic per-tenant write lock were on. It is not (see
 * AUTOMATIC_SITE_WRITE_LOCKS_ENABLED and backlog B-87), so the registry is always empty
 * and RsxLocks::_release_since() - which calls _forget_site_lock_tokens() on
 * Rsx_Site_Model_Abstract alone - has nothing to miss. Re-enabling those locks means
 * revisiting that call so it reaches every adopter.
 *
 * See: php artisan rsx:man model, rsx:man class_override
 */
trait Site_Scoped
{
    /**
     ...
     */
    protected static function bootSite_Scoped()
    {
