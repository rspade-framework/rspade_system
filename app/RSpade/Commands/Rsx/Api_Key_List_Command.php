 ...
 *
 * ACCESS ('read-only' / 'read+write') is a column for the same reason SCOPE is, and they
 * answer different halves of "why did this key get a 403": Access is the VERBS it may use,
 * Scope is the PATHS it may reach. A read-only key is refused before its scopes are read.
 *
...
class Api_Key_List_Command extends Command
{
    ...
    public function handle()
    {
        ...
        $rows = array_map(static fn (Api_Key_Model $key) => [
            ...
            Api_Key_Cli_Support::key_access_summary($key),
        ...
        $this->newLine();
        $this->table(
            ['Id', 'Name', 'Prefix', 'State', 'Access', 'Scope', 'Expires', 'Last used', 'Created'],
            $rows
        );
