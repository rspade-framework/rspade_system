...
class Contact_Model extends Rsx_Site_Model_Abstract
{
     ...
     * generated Base_Contact_Model.js declares each one. Hand-adding the keys inside fetch()
     * would put them on exactly one payload and on none of the others.
     *
     * portal_user_id RESOLVES A ROW to answer (see resolve_portal_user()), so a payload
     * carrying many contacts pays a lookup per record. It is declared here anyway because the
     * alternative - each surface recomputing it - is how the answers drift; a caller that
     * needs a wide list without it selects the columns it wants rather than serializing.
     *
     * @var array
     */
    protected $appends = ['contact_id_formatted', 'full_name', 'portal_user_id'];
    ...
    /**
     * Accessor for the appended `contact_id_formatted` property. Delegates - never
     * re-implements: the method is the definition, the property is only its serialization.
     *
     * @return string
     */
    public function getContactIdFormattedAttribute(): string
    {
        return $this->contact_id_formatted();
    }
    ...
    /**
     * Accessor for the appended `full_name` property. Delegates - never re-implements.
     *
     * @return string
     */
    ...
    /**
     * Accessor for the appended `portal_user_id` property. Delegates - never re-implements.
     *
     * @return int|null
     */
    public function getPortalUserIdAttribute()
    {
    ...
    public static function fetch($id)
    {
        ...
        // No hand-added keys: contact_id_formatted, full_name and portal_user_id are declared
        // derived properties ($appends above), so toArray() already carries them - including
        // the portal user id the "View as Client" affordance reads.
