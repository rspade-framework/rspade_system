...
class Client_Model extends Rsx_Site_Model_Abstract
{
     ...
     * generated Base_Client_Model.js declares each one. Hand-adding the keys inside a payload
     * builder would put them on that ONE payload and on none of the others.
     *
     * country_name, region_name and portal_member_count each READ A ROW to answer. They are
     * declared here anyway because the alternative - each surface recomputing them - is how
     * the answers drift; a caller that needs a wide list without them selects the columns it
     * wants rather than serializing.
     *
     * get_created_by_author() is deliberately NOT here: it resolves an actor and a
     * realm-gated profile URL for one viewer, which is a question a list payload should not
     * be asking on every row. to_fetch_array() asks for it explicitly instead.
     *
     * @var array
     */
    protected $appends = ['country_name', 'client_id_formatted', 'region_name', 'portal_member_count'];
    ...
    /**
     * Accessor for the appended `country_name` property. Delegates - never re-implements:
     * the method is the definition, the property is only its serialization.
     *
     * @return string|null
     */
    ...
    /**
     * Accessor for the appended `client_id_formatted` property. Delegates - never
     * re-implements.
     *
     * @return string
     */
    public function getClientIdFormattedAttribute(): string
    {
        return $this->client_id_formatted();
    }
    ...
    /**
     * Accessor for the appended `region_name` property. Delegates - never re-implements.
     *
     * @return string|null
     */
    ...
    /**
     * Accessor for the appended `portal_member_count` property. Delegates - never
     * re-implements.
     *
     * @return int
     */
    public function getPortalMemberCountAttribute(): int
    {
        return $this->portal_member_count();
    }
    ...
    public function to_fetch_array(): array
    {
        ...
        // region_name, country_name, client_id_formatted and portal_member_count are declared
        // derived properties ($appends above), so toArray() already carried them.
