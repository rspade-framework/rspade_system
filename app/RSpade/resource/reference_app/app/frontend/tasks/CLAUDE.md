...
- **`description` is a declared `Rich_Text` column** on `Task_Model`: the edit form names no widget (`<{Task_Model.editor_for('description')} />`, which resolves to the WYSIWYG) and its form-data default is `null`, never `''`; the view page prints the value itself, guarded by `is_empty()`. The datagrid's `filter` still searches `tasks.description` with a `LIKE` over the stored markup.
