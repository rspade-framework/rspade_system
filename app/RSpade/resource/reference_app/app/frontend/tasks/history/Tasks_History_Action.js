/**
 * Tasks revision-history action
 *
 * The read-only change timeline for one task: Page_Scaffold + the entity header the
...
@auth('is_logged_in')
class Tasks_History_Action extends Spa_Action {
    ...
    async on_load() {
        try {
            this.data.record = await Task_Model.fetch(this.args.id);
        } catch (e) {
    ...
    async page_title() {
        ...
        return (this.data.record.title || 'Task') + ' - History';
    }

    async breadcrumb_label() {
        await this.await_loaded();
        return (this.data.record.title || 'Task');
    }
    ...
    page_actions() {
        return `
            <div class="d-flex gap-2">
                <a href="${Rsx.Route('Tasks_View_Action', this.args.id)}" class="btn btn-secondary btn-sm">
