/**
 * Clients_DataGrid - quick filters (status, priority) in the card header.
 */
class Clients_DataGrid extends DataGrid_Abstract {
    static allowed_filters = ['status_id', 'priority'];

    static record_noun_plural = 'clients';
    ...
    async _delete_selection(selection) {
        ...
        const result = await Frontend_Clients_Controller.bulk_delete({
            mode: selection.mode,
            ids: selection.ids,
    ...
    async export_selection(selection) {
        const result = await Frontend_Clients_Controller.export_csv({
            mode: selection.mode,
            ids: selection.ids,
            filter_params: selection.filter_params,
        });
