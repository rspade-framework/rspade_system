/**
 * Contacts_DataGrid - quick filter (priority) in the card header.
 */
class Contacts_DataGrid extends DataGrid_Abstract {
    static allowed_filters = ['priority'];
    ...
    async _delete_selection(selection) {
        ...
        const result = await Frontend_Contacts_Controller.bulk_delete({
            mode: selection.mode,
            ids: selection.ids,
    ...
    async export_selection(selection) {
        const result = await Frontend_Contacts_Controller.export_csv({
            mode: selection.mode,
            ids: selection.ids,
            filter_params: selection.filter_params,
        });
        ...
        Flash_Alert.success('Exported ' + result.count + ' contact' + (result.count === 1 ? '' : 's'));
    }
