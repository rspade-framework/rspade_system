...
|----|--------------------------|------|-------|-------------------|--------|--------------|
| M-01 | Maintenance grants a CLUSTER lock as a NO-OP that leaves nothing behind. REVISED 2026-08-11 (owner ruling): this row previously pinned the flock degradation, which bought exclusion against a peer maintenance had already removed and cost real php-fpm contention | php | `named_write_lock('rsxtest noop/name')` under forced maintenance | token prefixed `maint:`; NO file at `storage/state/flock/cluster__rsxtest_noop_name.lock` | implemented | 2026-08-11 |
...
| M-20 | Web answers a 503 page quoting the reason | cli (curl) | `curl -si http://localhost/` while up | HTTP 503, `Content-Type: text/html`, title `Maintenance in progress`, body contains the reason and the disable hint, never the mode stamp | implemented (skips without a local server) | 2026-09-21 |
...
| M-32 | THE BEHAVIOR: the block's own lines (extracted from the script, never retyped) reap real daemons matched only by the socket path in their argv - including a WEDGED one that ignores SIGTERM, which is what makes the KILL pass load-bearing | cli | two node processes with `--socket=<scratch>/fake.sock`, one cooperative + one trapping SIGTERM; script's block run with `rsx_tmp_root()` pointed at the scratch root | "Quiesced 2 node RPC helper daemon(s)"; both processes ended by TERM/KILL; pgrep count 0 | implemented | 2026-08-13 |
