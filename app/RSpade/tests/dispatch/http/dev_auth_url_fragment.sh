...
echo "[TEST] 1. rsx:debug /contacts --user=1 renders authenticated..." >&2
plain="$(php artisan rsx:debug "/contacts" --user=1 2>&1)"
...
echo "[TEST] 2. rsx:debug '/contacts#foo=bar' --user=1 renders authenticated..." >&2
fragment="$(php artisan rsx:debug "/contacts#foo=bar" --user=1 2>&1)"
...
if ! echo "$fragment" | grep -q '/contacts#foo=bar'; then
