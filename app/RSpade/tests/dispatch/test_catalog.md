...
| DISP-03 | B4.6: AUTH_REQUIRED full-page GET redirects to login with intended URL | php | `Error_Response(ERROR_AUTH_REQUIRED)`, GET `/dashboard`, logged out | `RedirectResponse` to `/login` containing `redirect=%2Fdashboard` | implemented | 2026-07-30 |
...
| DISP-07 | Wire-level split: logged-out full-page GET 302s to login with `?redirect=` | http | GET `/dashboard` (logged out) | 302, `Location: /login?redirect=%2Fdashboard` | implemented | 2026-07-30 |
| DISP-08 | Wire-level split: same rejection via ajax endpoint keeps JSON contract | http | POST `/_ajax/Frontend_Dashboard_Controller/dashboard_data` (logged out) | no redirect, body has `error_code:unauthorized` | implemented | 2026-07-30 |
| DISP-09 | Dev auth: a `#fragment` in the rsx:debug url is not signed - the harness still renders AUTHENTICATED | http | `rsx:debug /contacts` and `rsx:debug '/contacts#foo=bar'` with `--user=1` | both bodies carry `"is_auth": true`; the fragmented run navigates to `/contacts#foo=bar` | implemented | 2026-08-17 |
...
| DISP-24 | The signed payload is the documented wire format (key order + PHP `\/` escaping) | php | `Dev_Auth_Token::payload()` | exact JSON string | implemented | 2026-09-06 |
