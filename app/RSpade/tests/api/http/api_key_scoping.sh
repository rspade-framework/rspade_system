...
# real HTTP. This test mints two keys - one scoped to /api/v1/contacts/*, one with NULL
...
# ---------------------------------------------------------------------------
echo "[TEST] 1. Scoped key -> reachable endpoint 200..." >&2
status=$(curl -s -o /tmp/api_scope_body.txt -w '%{http_code}' \
    -H "Authorization: Bearer $SCOPED_KEY" "$BASE/api/v1/contacts")
[ "$status" = "200" ] || fail "reachable endpoint expected 200, got $status"
grep -q '"items"' /tmp/api_scope_body.txt || fail "200 body missing items key"
echo "[TEST] 1. OK" >&2
...
status=$(curl -s -o /tmp/api_scope_body.txt -w '%{http_code}' \
    -H "Authorization: Bearer $SCOPED_KEY" "$BASE/api/v1/clients")
...
tr -d '\\' < /tmp/api_scope_body.txt | grep -q '"required":"/api/v1/clients"' \
...
# '/api/v1/contacts/*' is prefix-inclusive and method-blind, so the POST create endpoint
# under it is reachable. It answers 422 for an empty body, which is the endpoint's own
...
status=$(curl -s -o /dev/null -w '%{http_code}' \
    -H "Authorization: Bearer $SCOPED_KEY" "$BASE/api/v1/clients?bogus=1")
...
# The scoped key's own endpoint still answers 200 with a query string attached, and the
# unreachable one still answers 403 with one - the scope sees the path only.
# ---------------------------------------------------------------------------
echo "[TEST] 4b. The query string is ignored by the scope check..." >&2
status=$(curl -s -o /dev/null -w '%{http_code}' \
    -H "Authorization: Bearer $SCOPED_KEY" "$BASE/api/v1/contacts?page=1")
[ "$status" = "200" ] || fail "a query string must not change the scope answer, got $status"
echo "[TEST] 4b. OK" >&2
...
status=$(curl -s -o /dev/null -w '%{http_code}' \
    -H "Authorization: Bearer $OPEN_KEY" "$BASE/api/v1/clients")
[ "$status" = "200" ] || fail "unrestricted key expected 200 on /clients, got $status"
status=$(curl -s -o /dev/null -w '%{http_code}' \
    -H "Authorization: Bearer $OPEN_KEY" "$BASE/api/v1/contacts?bogus=1")
...
dev_db "UPDATE _api_keys SET scopes = '/api/v1/contacts/*' WHERE id = $SCOPED_KEY_ID"
echo "[TEST] 7. OK" >&2
...
handler=$(dev_db "SELECT handler FROM _api_request_log WHERE id > $LOG_BASELINE AND status = 403 AND response_error_code = 'insufficient_scope' AND path = '/api/v1/clients' LIMIT 1")
...
for path in /api/v1/contacts /api/v1/clients /api/v1/me; do
    status=$(curl -s -o /tmp/api_scope_body.txt -w '%{http_code}' \
...
if [ -f "$LARAVEL_LOG" ]; then
    tail -c +$((LOG_SIZE_BEFORE + 1)) "$LARAVEL_LOG" \
        | grep -q "API key #$SCOPED_KEY_ID: ignoring malformed scope '/api/v1/contacts\*'" \
...
for path in \
    "/api/v1/Contacts_Api_Controller/index" \
    "/api/v1/Contacts_Api_Controller::index" \
