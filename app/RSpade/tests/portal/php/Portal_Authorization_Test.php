...
/**
 * Tests for the T1 portal authorization contract:
 *   - Portal_Permission (identity, membership/role, can_collaborate)
 *   - portal_can_read() per-model visibility rules (own-record, shared-recipient,
 *     membership-scoped) and the Portal_Authorizable::portal_fetch() gate.
 *
 * Site-scoped app models (Client/Contact/Membership/Project/Shared) scope by the
 * STAFF Session site_id, so each test sets that via __acting_as_site() for seeding
 * and queries; the portal IDENTITY is set via Portal_Session CLI setters. Runs in
 * the default per-test transaction (rolled back afterward).
 */
class Portal_Authorization_Test extends Rsx_Test_Abstract
{
    ...
    {
        // The client-portal schema (portal_memberships, shared_items,
        // portal_projects, portal_users.contact_id) is provisioned by the
        // migration pipeline from database/migrations/rspade/ - see F-MIG /
        // issues_encountered.md ISSUE-1. Align the staff Session site so
        // site-scoped models seed/query on SITE_ID.
    ...
    // =====================================================================
    // membership-scoped (Portal_Membership_Model / Portal_Project_Model)
    // =====================================================================
    ...
    // =====================================================================
    // shared-recipient (Shared_Item_Model::portal_can_read)
    // =====================================================================
