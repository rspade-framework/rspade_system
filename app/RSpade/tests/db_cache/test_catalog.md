...
| DBC-12 | An empty database restores the cache and applies nothing further | cli | test DB dumped to a sandbox cache, then dropped; development-mode child with `--_no-snapshot` (no source regeneration) | restore fires; tables/migrations/users round-trip; no `Migrating:` | implemented | 2026-08-24 |
...
| DBC-15 | The command never runs outside development mode, and the refusal touches nothing (no maintenance window, no marker, no blob backup, no database change) | cli | subprocess with `RSX_MODE=debug`; this box stays in development | exit 1 carrying the unsealed-build refusal (the seal gate answers before handle() on an unsealed production-like box) and naming `rsx:build --force`; flag/marker/backup absent; table + migration counts unchanged | implemented | 2026-09-15 |
