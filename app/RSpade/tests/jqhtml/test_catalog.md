...
| JQ-CAS-02 | The form loading overlay paints the registered spinner with no deferral - the canonical cascade mount | playwright | a runtime-registered component that mounts `Rsx_Form` on a node appended during its OWN first render and arms `set_loading(true)` there, sampled while loading | the overlay's spinner host carries a live component instance that painted, at first observation | implemented | 2026-09-08 |
