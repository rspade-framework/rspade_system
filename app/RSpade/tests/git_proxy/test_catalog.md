...
| GP-21 | A CONFLICTED gitlink is reported, never resolved - which release to run is a decision, not a merge - and the recipe names the PROXY, not rsx:framework:pull (which advances to the latest release rather than to the chosen revision) | the three resolving commands printed; nothing automated; conflict left in place |
...
| GP-25 | The two APPEND-APPEND files are merged by the proxy and the operation completes: .migration_whitelist as the key-union of both sides' `migrations` maps, framework_update_history.dat as a line-union. Covers the merge path and the rebase path, which stops once per conflicting commit | exit 0; no unmerged paths; a merge commit on HEAD; the whitelist parses and holds ancestor + both new keys in sorted order; both [NOTE] lines printed |
| GP-26 | A conflict the proxy does NOT own stops the merge anyway: the whitelist is still merged and staged, the README stays conflicted, git's exit code stands and no merge commit is made | whitelist resolved + 3 entries; README still UU; MERGE_HEAD present; HEAD unmoved |
