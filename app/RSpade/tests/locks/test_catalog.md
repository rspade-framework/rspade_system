...
| POOL-01 | The pool lock grants in FIFO order; a parked pool.lock gets no frame until granted | http | holder + 3 queued connections (also the pure state machine) | granted one at a time in request order | implemented | 2026-09-26 |
| POOL-02 | EVERY pool op answers exactly one frame echoing its request id (errors included) | http | 10 pool frames + a ping fence on a raw socket | 11 frames, ids in order, nothing extra | implemented | 2026-09-26 |
| POOL-03 | count excludes the caller when it is a member; a non-member counts everyone | http | two members, counted by each and by a third | 1 / 1 / 2 | implemented | 2026-09-26 |
| POOL-04 | member_alive answers true for a live member, false for an unknown or departed one | http | live id, `pm_nobody`, a SIGKILLed member's id | true / false / false | implemented | 2026-09-26 |
| POOL-05 | Pools with different names are fully independent (lock and member set) | http | COUNT held while COUNT_OTHER is taken | granted; count 0; foreign member id not alive | implemented | 2026-09-26 |
| POOL-06 | unlock/join/leave/count/member_alive answer error to a caller not holding the lock; a second join is refused | http | each op without pool.lock; join twice | error frames; state unchanged | implemented | 2026-09-26 |
| POOL-07 | A SIGKILLed member stops being a member with no leave ever sent | http | `kill -9` a member process | members 0; member_alive false | implemented | 2026-09-26 |
| POOL-08 | A SIGKILLed pool-lock HOLDER frees the lock to the next waiter, and its membership goes too | http | holder process with a waiter queued, `kill -9` | waiter granted; count 0 | implemented | 2026-09-26 |
| POOL-09 | A disconnected waiter leaves the queue; the lock passes over it | http | queued waiter's socket destroyed | next waiter granted on unlock | implemented | 2026-09-26 |
| POOL-10 | release_all frees pool state: lock granted onward, membership removed, parked pool.lock answered `error` | http | holder+member with a waiter; a raw parked pool.lock then release_all | granted onward; count 0; "Wait cancelled by release_all" | implemented | 2026-09-26 |
| POOL-11 | Pool waits are invisible to the deadlock detector (THE RULE makes that safe) | http | pure: a holds L waits pool P; b holds P asks L | parks, not refused; `conn.waits` empty | implemented | 2026-09-26 |
| POOL-12 | Pool names are validated; stats is unlocked; empty pools are garbage-collected; member ids are random | http | pure state machine | bad names refused; unknown pool reads empty; `pm_` + 32 hex | implemented | 2026-09-26 |
| POOL-13 | dump / stats / `lockd.js dump` carry pool state | http | pool held + member on a scratch daemon | pools list, per-connection pool view, pool_counters; CLI renders HELD/MEMBER POOL lines | implemented | 2026-09-26 |
