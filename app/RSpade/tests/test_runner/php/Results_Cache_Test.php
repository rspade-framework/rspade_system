...
/**
 * The RESULT CACHE: a docker run records its per-class records and exit code under the
 * manifest build key AND the selector it ran, and a later run under both replays that
 * record instead of running again. The build key is the hash of every scanned source file,
 * so "same key" means "same code", and the recorded verdict is the verdict that code has.
 *
 ...
 * verdict onto different code); a missing or unreadable file is "not yet run". The
 * SELECTOR half of the key - which classes a verdict is for - is asserted in
 * Docker_Dispatch_Test, beside the dispatch rule that produces it.
 *
...
class Results_Cache_Test extends Rsx_Test_Abstract
{
    ...
    public static function test_a_written_record_reads_back_whole_under_its_key()
    {
        ...
        try {
            static::__invoke_static('write_cached_results', [$path, 'key_abc', 'sel_1', '/tmp/run_1', $by_class, 1]);
            $read = static::__invoke_static('read_cached_results', [$path, 'key_abc', 'sel_1']);
            ...
            static::__assert_equals('sel_1', $read['selector_key']);
    ...
    public static function test_a_record_under_another_key_is_not_a_hit()
    {
        ...
        try {
            static::__invoke_static('write_cached_results', [$path, 'key_abc', 'sel_1', '/tmp/run_1', [], 0]);

            static::__assert_null(static::__invoke_static('read_cached_results', [$path, 'key_xyz', 'sel_1']), 'a different build key never reads another key\'s verdict');
        } finally {
            @unlink($path);
        }
    }
    ...
    public static function test_missing_and_unreadable_records_mean_not_yet_run()
    {
        ...
        static::__assert_null(static::__invoke_static('read_cached_results', [$missing, 'key_abc', 'sel_1']), 'no file: no verdict');

        $garbage = static::__scratch_path();
        try {
            file_put_contents($garbage, '{not json');
            static::__assert_null(static::__invoke_static('read_cached_results', [$garbage, 'key_abc', 'sel_1']), 'unreadable file: no verdict');

            file_put_contents($garbage, json_encode(['build_key' => 'key_abc', 'selector_key' => 'sel_1']));
            static::__assert_null(static::__invoke_static('read_cached_results', [$garbage, 'key_abc', 'sel_1']), 'a record without by_class/exit_code is incomplete: no verdict');
        } finally {
            @unlink($garbage);
        }
    }
