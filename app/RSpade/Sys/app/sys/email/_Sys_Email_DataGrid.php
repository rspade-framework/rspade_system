...
/**
 * The Email screen's queue grid: every _email_queue row on the install, newest first.
 *
 * The caller (_Sys_Email_Controller::datagrid_fetch) runs fetch() outside the site
 * scope, so every tenant's rows are here.
 *
 * Filters (each a top-level request param, declared on _Sys_Email_DataGrid.jqhtml):
 ...
 * Search (filter) matches the recipient - the envelope address and, on a dev host, the
 * address the message was really for (dev_original_to) - or the subject, as a substring.
 *
 * The list payload omits the rendered bodies, headers and template data - the detail
 * screen reads them.
 */
class _Sys_Email_DataGrid extends _Sys_DataGrid_Abstract
{
    ...
    {
        $query = Email_Queue_Model::query()->select(self::LIST_COLUMNS);

        $status_id = _Sys_Enum_Words::enum_value(Email_Queue_Model::class, 'status_id', (string) ($params['status'] ?? ''));
        if ($status_id !== null) {
            $query->where('status_id', $status_id);
        }

        $category_id = _Sys_Enum_Words::enum_value(Email_Queue_Model::class, 'category_id', (string) ($params['category'] ?? ''));
        if ($category_id !== null) {
        ...
        if ($search !== '') {
            ...
            $query->where(function ($where) use ($like) {
                $where->where('to_address', 'like', $like)
    ...
    protected static function __transform_records(array $records, array $params): array
    {
        ...
        return array_map(function ($row) use ($names) {
            $row['status'] = _Sys_Enum_Words::enum_word(Email_Queue_Model::class, 'status_id', (int) $row['status_id']);
