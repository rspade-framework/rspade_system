/**
 * _Apidocs_Code_Samples - client-generated code samples (cURL / PHP / JS / Python / PowerShell)
 ...
 * doing so as the reader types. It reads _Apidocs_Tester.current_values(), the same method the
 ...
 */
class _Apidocs_Code_Samples extends Component {
    on_create() {
        this.state = {
            active: 0,
            fill: false,
    ...
    on_ready() {
        ...
        this.$.closest('._Apidocs_Endpoint_Card')
            .off('input.acs change.acs')
            .on(
                'input.acs change.acs',
                '._Apidocs_Tester__input, ._Apidocs_Tester__body, ._Apidocs_File_Tester__input, ._Apidocs_File_Tester__file',
                this._values_handler
            );
        ...
        this.$sid('panes').off('click.acs').on('click.acs', 'button._Apidocs_Code_Samples__copy', function () {
    ...
    _filled_values() {
        ...
        // contract, and the card decides which one is mounted (see _Apidocs_File_Endpoint_Card).
        const tester = this.$.closest('._Apidocs_Endpoint_Card').find('._Apidocs_Tester, ._Apidocs_File_Tester').component();
        ...
        values.api_key = Rsx_Storage.session_get(_Apidocs_Key_Bar.STORAGE_KEY) || '{API_KEY}';
    ...
    _activate(idx) {
        ...
        $tabs.find('button[data-idx]').removeClass('_Apidocs_Code_Samples__tab--active');
        $tabs.find('button[data-idx="' + idx + '"]').addClass('_Apidocs_Code_Samples__tab--active');

        const $panes = this.$sid('panes');
        $panes.find('._Apidocs_Code_Samples__pane').addClass('_Apidocs_Code_Samples__pane--hidden');
        $panes.find('._Apidocs_Code_Samples__pane[data-idx="' + idx + '"]').removeClass('_Apidocs_Code_Samples__pane--hidden');
    }
