/**
 * _Apidocs_Sidebar - endpoint filter + resource-grouped navigation.
 *
 ...
 */
class _Apidocs_Sidebar extends Component {
    on_create() {
        this.state = { filter: '' };
    }
    ...
    _apply_filter() {
        ...
        this.$.find('._Apidocs_Page__nav-group').each(function () {
            const $group = $(this);
            let group_visible = 0;

            $group.find('._Apidocs_Page__nav-link').each(function () {
                const $link = $(this);
                const matches = needle === '' || str($link.attr('data-search')).indexOf(needle) !== -1;

                $link.toggleClass('_Apidocs_Page__nav-link--filtered', !matches);
            ...
            $group.toggleClass('_Apidocs_Page__nav-group--filtered', group_visible === 0);
            visible += group_visible;
        });

        this.$sid('filter_clear').toggleClass('_Apidocs_Page__filter-clear--hidden', needle === '');
        ...
        this.$sid('filter_empty').toggleClass('_Apidocs_Page__filter-empty--hidden', !blame_filter);
    }
