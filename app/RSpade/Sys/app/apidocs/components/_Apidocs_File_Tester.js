/**
 * _Apidocs_File_Tester - the "Try it" panel for an endpoint that takes multipart/form-data.
 *
 * A SEPARATE COMPONENT, not a mode of _Apidocs_Tester. The generic tester's whole request model is
 ...
 * the card swaps this one in (_Apidocs_File_Endpoint_Card) and the two stay simple.
 *
 * WHAT IS SHARED, and why it is shared that way:
 *   - the adopted key, read from the SAME Rsx_Storage slot _Apidocs_Key_Bar writes and refreshed
 *     on the same 'apidocs:key_changed' event, so one key bar drives both testers;
 *   - the response pane, rendered by _apidocs_response.js under this component's own BEM
 *     prefix, so there is one look for "here is what the server said";
 *   - current_values(), the same contract _Apidocs_Code_Samples reads to render "Fill in values",
 ...
 */
class _Apidocs_File_Tester extends Component {
    on_create() {
     ...
     */
    _adopted_key() {
        return Rsx_Storage.session_get(_Apidocs_Key_Bar.STORAGE_KEY);
    }
    ...
    _refresh_key_hint() {
        ...
        if (this._adopted_key()) {
            $hint.removeClass('_Apidocs_File_Tester__key-hint--warn').text('Using the key from the bar above.');
        } else {
            $hint.addClass('_Apidocs_File_Tester__key-hint--warn').text('No API key set - paste one in the bar above.');
        }
    }
     ...
     *
     * SHARED WITH THE CODE SAMPLES, exactly as _Apidocs_Tester.current_values() is: "Fill in
     ...
     * _Apidocs_Code_Samples needs no idea which tester answered it.
     *
    ...
    async _send() {
        const ep = this.args.endpoint;
        const file_param = _Apidocs_Catalog.file_param(ep);
    ...
    _render_message(message, level) {
        api_render_response_note(this.$sid('response'), '_Apidocs_File_Tester', message, level);
    }
}
