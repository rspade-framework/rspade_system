/**
 * _Apidocs_Confirm_Dialog - the docs console's own confirmation dialog.
 *
 ...
 *
 *   const ok = await _Apidocs_Confirm_Dialog.ask({
 ...
 * @Instantiatable
 */
class _Apidocs_Confirm_Dialog extends Component {
    /**
     ...
     * overflow; hung off <body> it would sit outside ._Apidocs_Page, where the --rsx-*
    ...
    static async ask(options) {
        ...
        const $root = $('._Apidocs_Page');

        if (!$root.exists()) {
            throw new Error('_Apidocs_Confirm_Dialog.ask() was called outside the API docs console.');
        }

        const $host = $('<div>').appendTo($root);

        const dialog = $host.component('_Apidocs_Confirm_Dialog', {
            title: options.title,
            body: options.body,
    ...
    on_create() {
        ...
        if (!args.title || !args.body || !args.confirm_label || !args.cancel_label) {
            throw new Error('_Apidocs_Confirm_Dialog requires title, body, confirm_label and cancel_label.');
        }
