/**
 * _Apidocs_Key_Bar - the shared tester-key control for the docs console.
 *
 * Holds ONE key in session-scoped Rsx_Storage (key 'apidocs_tester_key'); every _Apidocs_Tester on
 ...
 */
class _Apidocs_Key_Bar extends Component {
    ...
    on_create() {
        this.state = {
            key: Rsx_Storage.session_get(_Apidocs_Key_Bar.STORAGE_KEY),
        };
    }
    ...
    _sync_action() {
        ...
        if (!$temporary.exists()) {
            $use.removeClass('_Apidocs_Key_Bar__btn--hidden');
            return;
        }

        const is_empty = str(this.$sid('input').val()).trim() === '';

        $temporary.toggleClass('_Apidocs_Key_Bar__btn--hidden', !is_empty);
        $use.toggleClass('_Apidocs_Key_Bar__btn--hidden', is_empty);
    }
     ...
     *
     * ASKED WITH THE FRAMEWORK APPLICATION'S OWN DIALOG (_Sys_Modal), never the
     * application's Modal: this is framework code running inside somebody else's page, and
     * an app's dialog system is not the framework's to depend on.
     *
     * The minted key is stored down the SAME path a pasted one takes, so 'apidocs:key_changed'
     * broadcasts and every _Apidocs_Tester on the page picks it up with no special case.
     */
    async _mint() {
        ...
        try {
            result = await _Apidocs_Controller.mint_temporary_key();
        } catch (e) {
    ...
    async _use() {
        ...
        try {
            response = await fetch(_Apidocs_Key_Bar.VERIFY_PATH, {
    ...
    _clear() {
        Rsx_Storage.session_remove(_Apidocs_Key_Bar.STORAGE_KEY);
        this.state.key = null;
        this._clear_error();
        this.render();
        this._broadcast();
    ...
    _store(k) {
        Rsx_Storage.session_set(_Apidocs_Key_Bar.STORAGE_KEY, k);
        this._broadcast();
    }
    ...
    _show_error(message) {
        this.$sid('input').addClass('_Apidocs_Key_Bar__input--invalid');
        this.$sid('error').text(message).removeClass('_Apidocs_Key_Bar__error--hidden');
    }

    _clear_error() {
        this.$sid('input').removeClass('_Apidocs_Key_Bar__input--invalid');
        this.$sid('error').addClass('_Apidocs_Key_Bar__error--hidden');
    }
}
