/**
 * _Apidocs_Catalog - synchronous reader over the endpoint catalog baked into the page.
 *
 * The catalog is shipped in window.rsxapp.page_data.api_catalog by the app's docs bundle
 * (see _Apidocs_Bundle::load_rsxapp_data), so every lookup here is SYNCHRONOUS. That is the
...
class _Apidocs_Catalog {
     ...
     */
    static exists() {
        const cat = _Apidocs_Catalog.raw();
    ...
    static newest_version() {
        const versions = _Apidocs_Catalog.versions();
    ...
    static resolve_version(requested) {
        ...
        return _Apidocs_Catalog.newest_version();
    }
    ...
    static groups(version) {
        ...
        _Apidocs_Catalog._assign_slugs();

        return cat.by_version[_Apidocs_Catalog.resolve_version(version)] || {};
    }
    ...
    static find_endpoint(version, slug) {
        ...
        const groups = _Apidocs_Catalog.groups(version);
    ...
    static _assign_slugs() {
        ...
        _Apidocs_Catalog._slugged = true;
    }
    ...
    static is_file_endpoint(ep) {
        return !!_Apidocs_Catalog.file_param(ep);
    }
    ...
    static endpoint_count(version) {
        ...
        for (const group of Object.values(_Apidocs_Catalog.groups(version))) {
     ...
     */
    static openapi_url() {
        return _Apidocs_Catalog.base_path().replace(/\/+$/, '') + '/openapi.json';
    }
    ...
    static home_url(version) {
        const resolved = _Apidocs_Catalog.resolve_version(version);
        const base = _Apidocs_Catalog.base_path();

        return resolved === _Apidocs_Catalog.newest_version()
            ? base
    ...
    static endpoint_url(version, ep) {
        ...
        if (resolved !== _Apidocs_Catalog.newest_version()) {
