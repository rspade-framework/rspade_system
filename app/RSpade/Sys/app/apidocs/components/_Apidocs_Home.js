/**
 * _Apidocs_Home - the landing pane of the API reference.
 *
 ...
 */
class _Apidocs_Home extends Component {
    on_ready() {
        const that = this;
    ...
    async _adopt() {
        ...
        try {
            await _Apidocs_Controller.adopt_tester_key({ key: key });
        } catch (e) {
        ...
        Rsx_Storage.session_set(_Apidocs_Key_Bar.STORAGE_KEY, key);

        window.location.reload();
    }

    async _forget() {
        await _Apidocs_Controller.forget_tester_key();
    ...
    _show_error(message) {
        this.$sid('key_error')
            .text(message)
            .removeClass('_Apidocs_Home__keygate-error--hidden');
    }

    _hide_error() {
        this.$sid('key_error').addClass('_Apidocs_Home__keygate-error--hidden');
    }
}
