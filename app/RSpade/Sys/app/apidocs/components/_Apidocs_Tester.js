/**
 * _Apidocs_Tester - inline "Try it" panel that fires a real request at the live endpoint.
 *
 ...
 * examples). The Send button reads the active key from Rsx_Storage (written by _Apidocs_Key_Bar),
 ...
 */
class _Apidocs_Tester extends Component {
    ...
    _refresh_key_hint() {
        ...
        const key = Rsx_Storage.session_get(_Apidocs_Tester.STORAGE_KEY);
        if (key) {
            $hint.removeClass('_Apidocs_Tester__key-hint--warn').text('Using the key from the bar above.');
        } else {
            $hint.addClass('_Apidocs_Tester__key-hint--warn').text('No API key set - paste one in the bar above.');
        }
    }
    ...
    async _send() {
        ...
        api_render_response(this.$sid('response'), '_Apidocs_Tester', res.status, res.statusText, ms, _Sys_Code_Pane.pretty(text));
    }

    _render_message(message, level) {
        api_render_response_note(this.$sid('response'), '_Apidocs_Tester', message, level);
    }
}
