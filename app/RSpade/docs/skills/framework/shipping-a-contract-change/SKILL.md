...
└─ no - it is ONE-TIME, triggered by this change
    └─ can you write "IF YOU DO NOTHING: <one concrete breakage>"?
        ├─ yes -> an upstream_changes document, carrying that line
...
A document exists ONLY when the end user must DO something to their environment or their own code BY HAND, or something concrete BREAKS if they do not. **It is NOT a changelog** - "what changed upstream" is already carried by the framework-update commit body / `git log -- system/` / `framework_update_history.dat`. Self-applying behavior needs no document, however large the change.
