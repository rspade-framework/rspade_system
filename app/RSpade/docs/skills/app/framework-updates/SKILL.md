...
**If a pull reports restored files, read the list. If it names migrations, run `php artisan migrate`** - that box has been running current framework code against an older schema. (That is the a downstream deployment 2026-08-11 shape: 15 framework files lost from a current install, 13 of them migrations, while every command reported the tree healthy.)
