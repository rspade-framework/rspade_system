...
**External mirroring is NOT a sealed-build difference** - every mode mirrors external assets into `rsx/resource/.cdn-cache/` (git-tracked) and serves them from `/_vendor/`, never inlined into bundles. The sealed difference is only WHO MAY DOWNLOAD: **a sealed web request never does**, and a missing mirror file THROWS naming the file and `rsx:prod:refresh`. (A development miss is a 404 naming `rsx:cdn_externals:refresh`, which itself refuses on a sealed host.) The production build still runs its own explicit `[2/5] Mirroring external assets` step over `*.externals.php` declarations, because nothing renders those at build time. See skill `rspade:external-resources`.
