...
**Connection state**: one socket per tab; outgoing messages are batched into one array frame (the relay accepts no other shape). `disconnected` is announced only after a 5s grace with no reconnect - `reconnecting` fires immediately - so a blip never flashes offline; after a trying state, `disconnected` means a CONFIRMED outage, otherwise it means idle/lazy. An idle socket closes 10s after the last watch stops (the window spans an SPA navigation). A wall-clock gap (suspend/resume) or an unanswered liveness ping forces a reconnect, because resync is the only recovery from a link that may have missed messages.
