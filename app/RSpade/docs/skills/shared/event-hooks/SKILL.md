...
- **`migrate.normalize_schema.complete`** - fired at the end of every `migrate`, after the POST-migration `migrate:normalize_schema` pass and before the initial user and the revision dictionary, with NO payload. The schema is at its framework-normalized tip and the run's snapshot still covers what a handler does, so a throw rolls the migration back. This is the seam for an app's own "columns every table must have" pass, computed over `information_schema` instead of a hand-written per-table migration list. Handlers run inline in the migrate process and MUST be idempotent - it fires on every run, including one with nothing to migrate. See `rsx:man migrations` (POST-NORMALIZATION APP STEPS).
