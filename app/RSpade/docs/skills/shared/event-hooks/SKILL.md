...
- **Manifest lifecycle**: `rsx.rebuilt`, then the mode variant `rsx.rebuilt.dev` | `rsx.rebuilt.prod` (both only when THIS process rebuilt), then **`rsx.ready`** on every boot - all fired at the end of `Manifest::init()`. In development the rebuild happens on the first request after a source change; in production it is the deploy-time build. Handlers run inline at boot, so keep them tiny. An application artifact that belongs with the build (a sitemap, a compiled lookup table) is written from an `rsx.rebuilt` handler into `Rsx_Project_Paths::build_path('app/<name>')` - worked example and the rules in `rsx:man event_hooks`.
...
- **`migrate.normalize_schema.complete`** - fired after EVERY `migrate:normalize_schema` pass: once before any migration, once after each individual migration, and once at the end - so a run with N pending migrations fires it N+1 times and a run with nothing pending fires it twice. No payload. The final firing sits after the POST-migration pass and before the initial user and the revision dictionary. The schema is at its framework-normalized tip and the run's snapshot still covers what a handler does, so a throw rolls the migration back. This is the seam for an app's own "columns every table must have" pass, computed over `information_schema` instead of a hand-written per-table migration list. Handlers run inline in the migrate process and MUST be idempotent - it fires several times per run, and on a run with nothing to migrate. Firing after every pass is what makes an incremental box and a from-zero replay (a fresh install, a test database, `rsx:db:rebuild_provision_cache_snapshot`) produce the same schema at every step: a mid-history migration that reads a hook-computed column finds it there in both. See `rsx:man migrations` (POST-NORMALIZATION APP STEPS).
- **`user.initial.created`** - fired ONCE per database, when the application's first account is created by `Rsx_Initial_User::create()` (the first-run screen, the `RSPADE_DEFAULT_*` post-migrate step, the test-baseline seed, or a direct call). Payload `{user, login_user, site_id, source}`; both records are **id 1 by contract**, and `source` is `first_run | post_migrate | test_baseline | manual`. This is where an app attaches its founder's rows (a group membership, admin ACLs) - **never a migration that inserts rows keyed to user id 1**, which runs at a fixed point in history and never runs for the test baseline. It fires for the test baseline too, so handler rows are part of every test's starting database (changing one needs `Rsx_Test_Command::CACHE_VERSION` bumped or `rsx:test --fresh`). See `rsx:man initial_user`.
