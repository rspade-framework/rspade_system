---
name: form-input-contract
description: "Writing a form input on the framework's Form_Input_Abstract contract - _get_value()/_set_value(), _mark_ready() and value buffering, _notify_input() for user edits, the data-name attribute, timing indifference, and why an input never validates. Use when building an input widget, extending an existing input, wiring an input's events, or hitting \"must implement _get_value()\", \"must implement _set_value()\", \"child input has no data-name attribute\", a value that never appears, or a value lost on re-render."
---
...
(framework core, `system/app/RSpade/Core/Forms/Form_Input_Abstract.js`) owns the whole
value contract - buffering, `val()`, events, the `data-name` attribute - so a concrete
input only describes how its value maps to its DOM. An application never carries a copy
of the base class. The concrete inputs an application ships, and how to add one to that
roster, are the app skill `form-components`.
...
| Input shape | Call `_mark_ready()` |
|---|---|
| plain text input | `on_render()` |
| backed by a third-party widget library | after the library instantiates |
| server-fed select | after the options arrive |
| rich-text editor | inside the library's ready callback |

```javascript
on_ready() {
    const that = this;
    editor_ready(function () {
        that._initialize_editor();
...
drawn once imperatively vanishes on the next render.
...
`$name` goes on the INPUT, never on a field wrapper. A field without one raises
...
`super._set_value(value)` - it never becomes a `val()` override. An override of
`on_ready()` calls `super.on_ready()`, which is where the parent instantiates its widget
and calls `_mark_ready()`.
...
Details: `php artisan rsx:man form_input`, `rsx:man form_conventions`. Related:
`rspade:form-engine`, `rspade:jqhtml`, and the app skill `form-components` (the shipped
input roster and how to add one).
