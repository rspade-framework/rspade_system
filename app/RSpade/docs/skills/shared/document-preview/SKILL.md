...
**The binaries can be sandboxed, and on a site that accepts outside uploads they should be.** `LIBREOFFICE_SANDBOX=docker` (`rsx.libreoffice.sandbox`) routes every soffice and pdftotext spawn through `Document_Sandbox::command()`, which wraps it in one throwaway `docker run --rm` per file - no network, read-only root, `--cap-drop ALL`, no-new-privileges, memory and pid caps, tmpfs `/tmp`, the worker's own uid:gid, and the per-invocation work dir as the ONLY mount (bound at its own absolute path, so the argv is identical in both modes). The host then needs no LibreOffice and no poppler-utils; what it needs is a docker daemon the worker's user can reach. The image is the framework's own minimal converter image `rspade/rspade-docconvert` (soffice, pdftotext and fonts on a bare distribution base, nothing else), **built on first use** - so switching the posture on installs nothing, and `php artisan rsx:heal document-sandbox-image` is how you pay that cost during a deploy instead; an image of your own is yours to pull. `rsx:health` carries a **Document Sandbox** row - INFO for `none` in development, WARN for `none` on a sealed box, and five rows (client, daemon, image, one real hardened run, memory cap) when it is on, of which an absent *framework* image is only a WARN. **PhpSpreadsheet is not covered**: a workbook is read in-process, so there is no process to contain. Details: `rsx:man libreoffice`.
