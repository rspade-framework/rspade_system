...
**Security is two layers**: `#[Ajax_Endpoint_Model_Fetch]` makes the surface exist, and its MANDATORY `#[Auth]` is evaluated BEFORE any model code — **a denial returns the same generic "not found" as a missing row (anti-enumeration)**. Gates take no arguments, so **record-level rules (ownership, membership, record state) stay in the `fetch()` body**. **`fetch()` is for SECURITY, not aliasing** — never rename a field or format a date in it; it serves LIVE records only; **changes to it require user approval**. A model lacking `fetch()` gets one (a framework model by declaring `class X_Model extends X_Model_Abstract` in `rsx/models/` — core models are extended, never copied) — **separate controller endpoints that fetch single records are an anti-pattern**, and a model missing from the JS bundle means STOP and ask.

**Derived properties** — a computed value that must reach JS is `$appends` plus a `getXAttribute()` accessor DELEGATING to the public method that defines it (`getIsDocumentAttribute()` -> `is_document()`), so the method stays the definition and the property is only its serialization. Eloquent applies `$appends` inside `parent::toArray()`, so the value rides EVERY payload the model produces — `fetch()`, a relationship, a list — and the generated `Base_*_Model.js` declares each name as a read-only derived property (`field_length()` answers null for it; it is not a column). **Hand-adding a computed key inside `fetch()` is not the route**: it lands on that one payload and on no other, and nothing notices when a sibling model forgets. Worked example: `system/app/RSpade/resource/reference_app/models/party_model.php`. **An accessor runs on EVERY serialization, lists and batch fetches included** - one that reads other rows is paid per record, so a derivation that queries stays small and single-record in intent, and a list selects the columns it needs instead.

**Declared TEXT types**: a `public static $text_types = ['description' => Rich_Text::class]` map beside `$enums` says what KIND of string a TEXT column holds. A declared column is read as a **value object**, filtered by its type on every write, printed on a live page by the type's own component and reduced by `to_text()` for a CSV cell or a search index - so the encoding is stated once instead of being remembered at every place the column is printed, edited or exported. **An undeclared TEXT column is an ordinary string and is completely unaffected; declaration is opt-in.** The concrete types are APPLICATION code (the framework ships only `Rsx_Text_Abstract`); two required methods, `filter_set()` (the sanitizer) and `escape_string()` (plain text -> the encoding). **A bare string assigned to a declared column is PLAIN TEXT** - escaped by the type, then filtered - so an import or a plain API param never needs a hand-rolled `htmlspecialchars`; encoded content from server code is `Type::from_untrusted($html)`, and an API client sends it as the envelope JSON-encoded into a string param. **The value travels as an envelope `{__TEXT, raw, empty}` on every payload, the external API included, and is never flattened to a string for a consumer; an editor accepts only its own type and throws on a bare string.** **Use `is_empty()`, never `=== ''`** - a value object is never identical to a string, so the comparison is permanently false and an empty body reads as non-empty, silently. Skill `rspade:text-types`; `rsx:man text_types`.

**Column length**: `Model::field_length('col')` (PHP) and `Model.field_length('col')` (JS) are ONE definition - the max characters for a varchar/char, `null` for every other type, a throw for a column the model does not have. **A server-side length check reads it; it never restates a schema number**, and CTI detail columns answer too.
