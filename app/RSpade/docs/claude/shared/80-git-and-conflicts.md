...
To customize a framework class without modifying `system/`, **copy it into `rsx/` under the same class name** — copy-and-replace, not a subclass. The manifest uses your version, renames the framework file to `.upstream`, and keeps every reference to the old FQCN resolving. That `.php` <-> `.php.upstream` churn is always-local state and is never committed. **An override is a frozen COPY of a class that keeps moving: `CLASS-OVERRIDE-DRIFT-01` (`rsx:check`, HIGH) and the `rsx:health` "Class Override Drift" row name every public/protected member the archived file declares and your copy does not — and a SEAM (`Staff_Authorizable`, `Portal_Authorizable`, `fetch()`, `#[OnEvent]`) beats a clone whenever the addition is policy.** Details: `rsx:man class_override`, `rsx:man rsx_git`.
