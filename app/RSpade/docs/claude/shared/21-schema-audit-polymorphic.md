...
**`migrate` is fine to run whenever schema work needs it** — expected and encouraged after creating or changing a migration; treat it as one command (press the button), not "heavy machinery." **Always run `migrate` with its FULL output — never pipe it through `| tail`/`| head`, grep it, or truncate it in any way**: the snapshot/rollback narrative IS the diagnostic, and a truncated run hides which step failed. A migrate CANCELLED mid-run leaves its snapshot behind — `php artisan migrate:restore` performs the same restore a failed run performs automatically.

**A fresh database restores the shipped schema cache before migrating**: with NO TABLES AT ALL and `rsx/resource/db/schema_cache.sql.gz` present, `migrate` streams it in (plus `uploads_cache.tar.gz` into the blob store) and then **continues with the ordinary run** — a PRE-MIGRATE step, not a replacement, so migrations newer than the cache apply on top exactly as always. **A stale cache is therefore never a bug, and you NEVER regenerate it on your own initiative** — `rsx:db:dump_cache` is a destructive, operator-requested command (skill `rspade:migrations`).
