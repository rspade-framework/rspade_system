...
Background work is a `public static` Service method (a class extending `Rsx_Service_Abstract`, in `/rsx/services/`) marked `#[Task]`, taking `(Task_Instance $task, array $params = [])` and optionally `#[Schedule('daily at 3am')]`. Run one with `$id = Task::dispatch('Service', 'method', $params)` — enqueues, spawns a detached worker when due, returns a **pollable id** for `Task::status($id)`. One cron entry drives everything: `* * * * * php artisan rsx:task:process`. **A PHP script OUTSIDE the application boots the whole shell with `$app = require '<project>/system/script.php';` and nothing else** - same pre-boot guards, same manifest, same site: `rsx:man scripting`.

**An artisan command in RSpade is a `#[Task]` with a name.** An application writes NO command classes (all of `system/` is framework property) — it adds `#[Command('prefix:name', 'Description')]` beside the `#[Task]`, and the manifest-discovered declaration IS the command: `php artisan myapp:import` is `rsx:task:run <Service> <method>` under a friendlier name. **stdout is the VALUE (JSON, `--debug` wraps, exit 1 + JSON error on a throw); stderr is the NARRATION** — every `$task->info()`/`update_progress()` line live, `-q` silencing it and never the value. Skill `rspade:task-commands`; `rsx:man task_commands`.
...
Skills: `rspade:background-tasks` (the `Task_Instance` API, `#[Schedule]` phrases, the worker pool, retry semantics), `rspade:task-commands` (`#[Command]`, the console output contract, the build-time FATALs), `rspade:locks-and-subprocesses` (cluster vs system locks, semaphores, FIFO/deadlock refusal, lock groups). Details: `rsx:man tasks`, `rsx:man task_commands`, `rsx:man locks`.
