...
**`rsx:test` REFUSES to run in a production mode** (pre-boot, exit 1, naming `rsx:mode:set dev`): the suite is a CI step that runs on a development box before a deployment, and a sealed box used to answer a false green. **The `prod_mode` group runs ONLY when named** (`rsx:test --framework --group=prod_mode --sequential`) - its two classes switch the box's mode themselves; any class declaring `$explicit_group_only` is passed over by a bare run, which says so in one line.
